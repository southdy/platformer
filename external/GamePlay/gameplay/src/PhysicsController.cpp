...
#define FIXED_TIMESTEP (btScalar(1.) / btScalar(120.))
...
PhysicsController::PhysicsController()
  : _isUpdating(false), _enabled(true), _collisionConfiguration(NULL), _dispatcher(NULL),
...
void PhysicsController::drawDebug(const Matrix& viewProjection, const Rectangle& viewport)
{
    GP_ASSERT(_debugDrawer);
    GP_ASSERT(_world);

    _debugDrawer->begin(viewProjection, viewport);
...
{
public:
    PhysicsWorld(btDispatcher* dispatcher, btBroadphaseInterface* pairCache, btConstraintSolver* constraintSolver, btCollisionConfiguration* collisionConfiguration) :
...
void PhysicsController::initialize()
{
    ...
    _dispatcher->registerCollisionCreateFunc(BOX_2D_SHAPE_PROXYTYPE,
                                             BOX_2D_SHAPE_PROXYTYPE,
                                             bullet_new<btBox2dBox2dCollisionAlgorithm::CreateFunc>());
    _overlappingPairCache = bullet_new<btDbvtBroadphase>();
    _solver = bullet_new<btSequentialImpulseConstraintSolver>();

    // Create the world.
    _world = bullet_new<PhysicsWorld>(_dispatcher, _overlappingPairCache, _solver, _collisionConfiguration);
    _world->setGravity(BV(_gravity));
    _world->setForceUpdateAllAabbs(false);
    ...
    _actionInterface = new ActionInterface();
    _world->addAction(_actionInterface);
}
...
void PhysicsController::finalize()
{
    if(_world)
    {
        _world->removeAction(_actionInterface);
...
void PhysicsController::update(float elapsedTime)
{
    ...
    {
        PROFILE_(step);
        _world->stepSimulation(elapsedTime * 0.001f, MAX_SUBSTEPS, FIXED_TIMESTEP);
    }
    ...
    for (; iter != _collisionStatus.end(); iter++)
    {
        ...
        if ((iter->second._status & REGISTERED) != 0 && (iter->second._status & REMOVE) == 0 && iter->first.objectA->isEnabled() && (!iter->first.objectB || iter->first.objectB->isEnabled()))
        {
...
PhysicsCollisionShape* PhysicsController::createBox(const Vector3& extents, const Vector3& scale)
{
    ...
    for (unsigned int i = 0; i < _shapes.size(); ++i)
    {
        ...
        {
            btBox2dShape* box = static_cast<btBox2dShape*>(shape->_shape);
    ...
    shape = new PhysicsCollisionShape(PhysicsCollisionShape::SHAPE_BOX, bullet_new<btBox2dShape>(halfExtents));
...
Vector3 PhysicsController::interpolate(Vector3 const & previous, Vector3 const & current) const
{
    const float alpha = static_cast<PhysicsWorld*>(_world)->getLocalTime() / FIXED_TIMESTEP;
    return Vector3((current * alpha) + (previous * (1.0f - alpha)));
}

void PhysicsController::setEnabled(bool enable)
{
    _enabled = enable;
}

bool PhysicsController::isEnabled() const
{
    return _enabled;
}

float PhysicsController::getPhysicsTimeAlpha() const
{
    return static_cast<PhysicsWorld*>(_world)->getLocalTime() / FIXED_TIMESTEP;
}
...
PhysicsController::DebugDrawer::DebugDrawer()
    : _mode(btIDebugDraw::DBG_DrawAabb | btIDebugDraw::DBG_DrawConstraintLimits | btIDebugDraw::DBG_DrawConstraints | 
       btIDebugDraw::DBG_DrawContactPoints | btIDebugDraw::DBG_DrawWireframe), _lineBatch(NULL), _lineCount(0)
{
    _lineBatch = LineBatch::create();
}
...
void PhysicsController::DebugDrawer::begin(const Matrix& viewProjection, const Rectangle& viewport)
{
    _lineBatch->start(viewProjection, viewport);
}
...
void PhysicsController::DebugDrawer::drawLine(const btVector3& from, const btVector3& to, const btVector3& fromColor, const btVector3& toColor)
{
    ...
    _lineBatch->addLine(
                BV2GV(from),
                BV2GV(to),
                gameplay::Vector4(fromColor.x(), fromColor.y(), fromColor.z(), 1.0f),
                gameplay::Vector4(toColor.x(), toColor.y(), toColor.z(), 1.0f));
    ...
    if (_lineCount >= 4096)
    {
        ...
        _lineBatch->start(_viewProjection, _viewport);
    }
}
...
void PhysicsController::ActionInterface::updateAction(btCollisionWorld *collisionWorld, btScalar deltaTimeStep)
{
    Game::getInstance()->simulationUpdate(deltaTimeStep * 1000.0f);
}
