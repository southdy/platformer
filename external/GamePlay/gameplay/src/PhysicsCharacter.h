...
public:
    ...
    */
    ...
    void setClampFallToGravity(bool clamp);

    void setHorizontalImpulsesEnabled(bool enabled);

    void setGhostCollisionCallback(std::function<void(PhysicsCollisionObject*, Vector3)> ghostCallback);
...
private:
    ...
    void setNodeTranlsation(const Vector3& translation);

    static Vector3 getBlendedVector3(btVector3 const & previous, btVector3 const & current);
    ...
    btVector3 _previousVelocity;
    btManifoldArray _manifoldArray;
    float _slopeAngle;
    float _cosSlopeAngle;
    bool _physicsEnabled;
    bool _clampFallToGravity;
    bool _horizontalImpulsesEnabled;
    float _mass;
    ActionInterface* _actionInterface;
    std::function<void(PhysicsCollisionObject*, Vector3)> _ghostCallback;
};
