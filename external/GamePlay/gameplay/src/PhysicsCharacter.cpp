...
#define VEC3_WITH_LINEAR_FACTOR(vec3) Vector3(_linearFactor.x ? vec3.x : 0,\
    _linearFactor.y ? vec3.y : 0, _linearFactor.z ? vec3.z : 0)
namespace gameplay
{
...
PhysicsCharacter::PhysicsCharacter(Node* node, const PhysicsCollisionShape::Definition& shape, float mass, int group, int mask)
    ...
    _clampFallToGravity(false), _horizontalImpulsesEnabled(true)
{
...
PhysicsCharacter* PhysicsCharacter::create(Node* node, Properties* properties)
{
    ...
    PhysicsCharacter* character = new PhysicsCharacter(node, shape, mass,
                                                       properties->exists("group") ? properties->getInt("group") : PHYSICS_COLLISION_GROUP_DEFAULT,
                                                       properties->exists("mask") ? properties->getInt("mask") : PHYSICS_COLLISION_MASK_DEFAULT);
...
Vector3 PhysicsCharacter::getRenderPosition() const
{
    return Game::getInstance()->getPhysicsController()->interpolate(BV2GV(_previousPosition), BV2GV(_currentPosition));
}
...
void PhysicsCharacter::stepDown(btCollisionWorld* collisionWorld, btScalar time)
{
    ...
    if(_clampFallToGravity)
    {
        _verticalVelocity.setX(MATH_CLAMP(_verticalVelocity.getX(), std::min(-gravity.getX(), gravity.getX()), std::max(-gravity.getX(), gravity.getX())));
        _verticalVelocity.setY(MATH_CLAMP(_verticalVelocity.getY(), std::min(-gravity.getY(), gravity.getY()), std::max(-gravity.getY(), gravity.getY())));
        _verticalVelocity.setZ(MATH_CLAMP(_verticalVelocity.getZ(), std::min(-gravity.getZ(), gravity.getZ()), std::max(-gravity.getZ(), gravity.getZ())));
    }
    ...
    while (fraction > btScalar(0.01) && maxIter-- > 0)
    {
        ...
        if (callback.hasHit())
        {
            ...
            else
            {
                ...
                float const originalX = targetPosition.getX();
                updateTargetPositionFromCollision(targetPosition, BV(normal));

                if(!_horizontalImpulsesEnabled)
                {
                    targetPosition.setX(originalX);
                }
            }
        }
        else
    ...
    if (!_verticalVelocity.isZero() && time > 0.0f)
        _verticalVelocity = (targetPosition - _currentPosition) / time;
...
bool PhysicsCharacter::fixCollision(btCollisionWorld* world)
{
    ...
    for (int i = 0, count = pairCache->getNumOverlappingPairs(); i < count; ++i)
    {
        ...
        for (int j = 0, manifoldCount = _manifoldArray.size(); j < manifoldCount; ++j)
        {
            ...
            const bool isGhost = object->getType() == PhysicsCollisionObject::GHOST_OBJECT;
            const bool isKinematicSibling = object->isKinematic() && object->getNode()->getParent() == _node->getParent();
            if (!object || isGhost || isKinematicSibling)
            {
                if(isGhost && _ghostCallback && contactCount > 0)
                {
                    const btVector3& normal = manifold->getContactPoint(0).m_normalWorldOnB;
                    _ghostCallback(object, Vector3(normal.x(), normal.y(), normal.z()));
                }
                continue;
            }
...
void PhysicsCharacter::updateAction(btCollisionWorld* collisionWorld, btScalar deltaTimeStep)
{
    ...
    // When translated by another simulation step, undo its translation, it will be reapplied as part
    // of this step so that _previousPosition doesn't produce jittery motion
    btVector3 const otherSimulationStepTranslation = _ghostObject->getWorldTransform().getOrigin() - _currentPosition;
    translateNode(BV2GV(-otherSimulationStepTranslation));

    _previousPosition = _ghostObject->getWorldTransform().getOrigin();
    _previousVelocity = BV(getCurrentVelocity());

    // Re-apply the external translation
    translateNode(BV2GV(otherSimulationStepTranslation));
...
void PhysicsCharacter::setClampFallToGravity(bool clamp)
{
    _clampFallToGravity = clamp;
}

void PhysicsCharacter::setHorizontalImpulsesEnabled(bool enabled)
{
    _horizontalImpulsesEnabled = enabled;
}

void PhysicsCharacter::setGhostCollisionCallback(std::function<void(PhysicsCollisionObject*, Vector3)> ghostCallback)
{
    _ghostCallback = ghostCallback;
}

void PhysicsCharacter::setNodeTranlsation(const Vector3& translation)
{
    _node->setTranslation(VEC3_WITH_LINEAR_FACTOR(translation));
}

void PhysicsCharacter::transformChanged(Transform* transform, long cookie)
{
    PhysicsGhostObject::transformChanged(transform, cookie);
    if(!Game::getInstance()->getPhysicsController()->_isUpdating)
    {
        _currentPosition = _ghostObject->getWorldTransform().getOrigin();
...
void PhysicsCharacter::translateNode(const Vector3& translation)
{
    _node->translate(VEC3_WITH_LINEAR_FACTOR(translation));
}

void PhysicsCharacter::setLinearFactor(Vector3 linearFactor)
{
