...
class GameScriptTarget : public ScriptTarget
{
    ...
    GP_SCRIPT_EVENT(postSimulationUpdate, "f");
...
Game::Game()
    : _initialized(false), _state(UNINITIALIZED), _pausedCount(0),
      ...
      _profilerController(NULL), _timeScale(1.0f)
{
...
void Game::preSimulationUpdate(float elapsedTime)
{
    // stub
}

void Game::postSimulationUpdate(float elapsedTime)
{
    // stub
}

void Game::simulationUpdate(float elapsedTime)
{
    // stub
}
...
bool Game::startup()
{
    ...
    _profilerController = new ProfilerController();
    ...
    if (_properties)
    {
        ...
        else
        {
            ...
            if (sns)
            {
                ...
                GP_REG_GAME_SCRIPT_CB(postSimulationUpdate);
...
void Game::shutdown()
{
    ...
    if (_state != UNINITIALIZED)
    {
        ...
        SAFE_DELETE(_profilerController);
...
void Game::frame()
{
    ...
    if (_state == Game::RUNNING)
    {
        ...
        preSimulationUpdate(elapsedTime);
        ...
        // Application Update.
        postSimulationUpdate(elapsedTime);
        ...
        // Run script update.
        if (_scriptTarget)
            _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, postSimulationUpdate), elapsedTime);
	...
	else if (_state == Game::PAUSED)
    {
        ...
        // Script update.
        if (_scriptTarget)
            _scriptTarget->fireScriptEvent<void>(GP_GET_SCRIPT_EVENT(GameScriptTarget, postSimulationUpdate), 0);
