...
public:
     ...
     * @param firstNode The starting node, defaults to _firstNode
     */
    template <class T, class C>
    void visit(T* instance, bool (T::*visitMethod)(Node*,C), C cookie, Node* firstNode = NULL);
...
template <class T, class C>
void Scene::visit(T* instance, bool (T::*visitMethod)(Node*,C), C cookie, Node* firstNode)
{
    if(!firstNode)
    {
    ...
    {
        GP_ASSERT(firstNode->getScene() && firstNode->getScene() == this);
    }

    for (Node* node = firstNode; node != NULL; node = node->getNextSibling())
    {
